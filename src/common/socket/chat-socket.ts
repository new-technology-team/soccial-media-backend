...
export const isChatUserOnline = (userId: number) =>
  Boolean(onlineSocketsByUser.get(Number(userId))?.size);

export const getChatUserLastActiveAt = (userId: number) =>
  lastActiveByUser.get(Number(userId)) || null;
...
const trackCallParticipation = (userId: number, payload: any) => {
  ...
  const mode: "private" | "group" =
    payload?.mode === "group" ? "group" : "private";
...
const getOrCreateCallRoom = (
  ...
  if (existing) {
    existing.mode =
      existing.mode === "group" || mode === "group" ? "group" : "private";
  ...
  const room: ActiveCallRoom = {
    ...
    callType,
    initiatorId:
      Number(payload?.initiatorId || payload?.fromUserId || userId || 0) ||
      undefined,
...
const leaveActiveCallRoom = (userId: number, conversationId?: string) => {
  const targetConversationId =
    conversationId || activeCallByUser.get(userId)?.conversationId;
...
const relayCallEvent = (socket: Socket, eventName: string, payload: any) => {
  ...
  if (targetUserId) {
    if (
      !isChatUserOnline(targetUserId) &&
      ["call:offer", "call:incoming", "call:initiate"].includes(eventName)
    )
      return;
...
export const registerChatSocketHandlers = (server: Server) => {
  server.on("connection", (socket) => {
    ...
    socket.on("message:typing", (payload) =>
      relayTypingEvent(socket, payload, Boolean(payload?.isTyping)),
    );
    socket.on("typing", (payload) => relayTypingEvent(socket, payload, true));
    socket.on("stopTyping", (payload) =>
      relayTypingEvent(socket, payload, false),
    );
    ...
    socket.on("presence:check", (payload, ack) => {
      ...
      if (typeof ack === "function") {
        ack(
          targetUserId
            ? getPresencePayload(targetUserId)
            : {
                userId: 0,
                online: false,
    ...
    socket.on("call:initiate", (payload) =>
      relayCallEvent(socket, "call:incoming", payload),
    );
    ...
    socket.on("webrtc:offer", (payload) =>
      relayCallEvent(socket, "webrtc:offer", payload),
    );
    socket.on("webrtc:answer", (payload) =>
      relayCallEvent(socket, "webrtc:answer", payload),
    );
    socket.on("webrtc:ice-candidate", (payload) =>
      relayCallEvent(socket, "webrtc:ice-candidate", payload),
    );
    ...
    socket.on("call:room:acquire", (payload, ack) => {
      ...
      if (typeof ack === "function")
        ack({ conversationId, roomId: room.roomId });
    });
    socket.on("call:room:status", (payload, ack) => {
      const conversationId = String(payload?.conversationId || "").trim();
      const callSessionId = String(
        payload?.callSessionId || payload?.roomId || "",
      ).trim();
      ...
      if (typeof ack === "function") {
        ack(
          room
            ? {
                ...serializeCallRoom(room),
                active: true,
                requestedRoomId: callSessionId,
              }
            : {
                conversationId,
    ...
    socket.on("call:answer", (payload) => {
      ...
      if (userId && payload?.useJitsi) {
        joinActiveCallRoom(socket, payload);
      }
      ...
      notifyCallAnsweredOnOtherDevices(
        socket,
        { ...(payload || {}), answeredAt },
        answeredAt,
      );
    ...
    socket.on("call:leave", (payload) => {
      if (userId) {
        const room = leaveActiveCallRoom(
          userId,
    ...
    socket.on("call:end", (payload) => {
      const conversationId =
        String(payload?.conversationId || "").trim() ||
        activeCallByUser.get(userId)?.conversationId ||
        "";
      const endPayload = {
        ...(payload || {}),
    ...
    socket.on("call_ended", (payload) =>
      relayCallEvent(socket, "call_ended", payload),
    );
    ...
    socket.on("group_call_ended", (payload) => {
      const conversationId = String(payload?.conversationId || "").trim();
      if (conversationId)
        endActiveCallRoom(conversationId, {
          ...(payload || {}),
          fromUserId: userId || Number(payload?.fromUserId || 0) || undefined,
        });
    ...
    socket.on("participant_updated", (payload) => {
      ...
      if (conversationId && userId) {
        ...
        if (room && participant) {
          ...
          socket
            .to(conversationId)
            .emit("call:participants", serializeCallRoom(room));
        }
      }
    ...
    socket.on("participant_muted", (payload) => {
      ...
      if (conversationId && userId) {
        ...
        if (room && participant) {
          room.participants.set(userId, {
            ...participant,
            micMuted: payload?.micMuted !== false,
          });
          room.updatedAt = Date.now();
        }
      }
    ...
    socket.on("participant_speaking", (payload) =>
      relayCallEvent(socket, "participant_speaking", payload),
    );
